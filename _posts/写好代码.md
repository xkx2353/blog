...
3. 命名风格
   ...
   1. 类名使用UpperCamelCase风格，但以下情形例外:DO/BO/DTO/VO/AO/ PO / UID 等。
      ...
      ![DOOrDo](https://azou.tech/blog/static/image/DO_or_Do.png)
...
3. 注释太多和太少的问题，类和函数一定要写注释，而且要写得尽可能全面、详细，而函数内部的注释要相对少一些，一般都是靠好的命名、提炼函数、解释性变量、总结性注释来提高代码的可读性。
   ...
   *有注释，但是每一行内容过长*
   ...
   ![DOOrDo](https://azou.tech/blog/static/image/comment_but_long_raw.png)
...
2. 不允许任何魔法值(即未经预先定义的常量)直接出现在代码中。<!--我们的代码中还是存在不少魔法值的-->
...
> 多数情况人还是会出差错，我们还是要把这些事情交给工具吧

ali-check

![code_issue_by](https://azou.tech/blog/static/image/comment_but_long_raw.png)

SonarLint

![code_issue_by_sonar](https://azou.tech/blog/static/image/code_issue_by_sonar.png)
